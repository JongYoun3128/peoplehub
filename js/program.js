  // 전역 변수
        ...
        function checkIn() {
            ...
            // UI 업데이트
            document.getElementById('statusText').textContent = `${timeString}에 출근하셨습니다`;
        ...
        function checkOut() {
            ...
            // UI 업데이트
            document.getElementById('statusText').textContent = `${timeString}에 퇴근하셨습니다`;
        ...
        function renderAttendanceRecords() {
            ...
            if (recentRecords.length === 0) {
                container.innerHTML = '<p style="text-align: center; color: #718096; padding: 20px;">출퇴근 기록이 없습니다.</p>';
                return;
            }
        ...
        function initializeAttendance() {
            ...
            if (todayRecord) {
                // 오늘 이미 출근한 경우
                document.getElementById('statusText').textContent = `${todayRecord.checkIn}에 출근하셨습니다`;
                document.getElementById('checkInBtn').disabled = true;
                
                if (todayRecord.checkOut) {
                    // 이미 퇴근도 한 경우
                    document.getElementById('statusText').textContent = `${todayRecord.checkOut}에 퇴근하셨습니다`;
                    document.getElementById('workTime').textContent = todayRecord.totalWorkTime;
                    document.getElementById('checkOutBtn').disabled = true;
                } else {
                    // 출근은 했지만 퇴근 안한 경우
                ...
                // 오늘 아직 출근 안한 경우
                document.getElementById('statusText').textContent = '오늘 아직 출근하지 않았습니다';
        ...
        function viewEmployee(id) {
            ...
            const workYears = Math.floor((today - joinDate) / (365.25 * 24 * 60 * 60 * 1000));
        ...
        document.getElementById('employeeForm').addEventListener('submit', function(e) {
            ...
            if (!formData.name || !formData.department || !formData.position || 
                !formData.hireDate || !formData.phone || !formData.email) {
                alert('모든 필드를 입력해주세요.');
                return;
            }
